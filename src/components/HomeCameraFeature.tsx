      ...
      <div className="mx-auto max-w-[1500px] px-5 md:px-8 xl:px-10">
        ...
        <div className="grid items-center gap-8 lg:grid-cols-[1.05fr_1fr] lg:gap-16">
          <Link href={`/products/${product.slug}`} className="relative block aspect-square w-full overflow-hidden rounded-lg bg-white" aria-label={`View ${product.name}`}>
            <Image src={poster.src} alt={poster.alt || product.name} fill sizes="(max-width: 1023px) 100vw, 700px" className="object-contain" />
          </Link>
          <div>
            ...
            <h3 className="mt-4 text-3xl font-bold leading-tight md:text-4xl">Dahua {resolution}<br />{hasDualLight ? "Smart Dual Light kit" : "Two-camera PoE kit"}</h3>
            ...
            <dl className="mt-7 divide-y divide-zinc-300 border-y border-zinc-300">
              {[
                { icon: Cctv, label: `2 x ${resolution} cameras`, detail: attribute("Camera Model") },
                { icon: HardDrive, label: "4-channel recorder", detail: attribute("Recorder Model") },
