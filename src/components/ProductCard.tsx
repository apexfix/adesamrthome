...
export function ProductCard({ product }: ProductCardProps) {
  ...
  const currentPrice = currentRawPrice 
    ? (parseInt(currentRawPrice) > 1000 ? (parseInt(currentRawPrice) / 100).toFixed(2) : parseFloat(currentRawPrice).toFixed(2)) 
    : null;

  // 获取原价 (用于划线)
  const regularRawPrice = product?.prices?.regular_price || product?.regular_price;
  const regularPrice = regularRawPrice 
    ? (parseInt(regularRawPrice) > 1000 ? (parseInt(regularRawPrice) / 100).toFixed(2) : parseFloat(regularRawPrice).toFixed(2)) 
    : null;

  // 判断是否真的在打折 (原价必须大于现价)
  const isDiscounted = regularPrice && currentPrice && parseFloat(regularPrice) > parseFloat(currentPrice);
  ...
  return (
    <div className="group relative border rounded-lg overflow-hidden bg-white shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="p-4">
        {/* 商品名称 */}
        <h3 className="text-sm md:text-base font-bold text-gray-900 line-clamp-2 mb-1 h-10 md:h-12">
          ...
          <div className="flex flex-col">
            ...
            {isDiscounted ? (
              ...
              <span className="text-[#c5a47e] text-lg md:text-xl font-bold">
                {currentPrice ? `${currencySymbol}${currentPrice}` : "Enquire"}
              </span>
            )}
            
          </div>

          {/* 购买按钮换成了更小巧的图标或文字，适应一行5个的紧凑布局 */}
          <span className="inline-flex items-center justify-center px-3 py-1.5 bg-black text-white text-xs font-semibold rounded group-hover:bg-[#c5a47e] transition-colors duration-300 z-10 relative">
            View Details
          </span>
        </div>
      </div>
